...
#     def __getitem__(self, idx):
#         return {
#             'acc_sample': torch.from_numpy(self.samples['acc_samples'][idx, :]),
#             'gyr_sample': torch.from_numpy(self.samples['gyr_samples'][idx, :]),
#             'mag_sample': torch.from_numpy(self.samples['mag_samples'][idx, :]),
#             'surface_label': torch.from_numpy(self.samples['surface_labels'][idx])
#         }
#         # pass
...
#         for subject in tqdm(range(1, 1+1), desc='Loading Subjects'): #self.config_dict['SUBJECTS']
...
#         surface_label = self.onehot_encoder.fit_transform(self.label_encoder.fit_transform(surface_labels).reshape(-1,1)).toarray()

#         self.samples['acc_samples'] = np.array(acc_samples, dtype=np.float).transpose(0, 2, 1)
#         self.samples['gyr_samples'] = np.asarray(gyr_samples, dtype=np.float).transpose(0, 2, 1)
#         self.samples['mag_samples'] = np.asarray(mag_samples, dtype=np.float).transpose(0, 2, 1)
#         self.samples['surface_labels'] = np.asarray(surface_label, dtype=np.float)
        
#         logger.debug('acc_samples.shape: {}, gyr_samples.shape: {}, mag_samples.shape: {}, surface_labels.shape: {}'.format(
#             self.samples['acc_samples'].shape,
