...
def train_func(model, gait_loader, epoch, optimizer, loss_func):
    ...
    for step, (acc, gyr, mag, targets) in enumerate(gait_loader):
        ...
        print(
            f'epoch[{epoch+1}], step {step+1}: acc = {np.mean(accuracies): .5f}, \
...
def eval_func(model, gait_loader, loss_func):
    model.eval()
    losses = []
    accuracies = []
    ...
    print(f'val_acc = {np.mean(accuracies): .5f}, \
                val_loss = {np.mean(losses): .5f}')
    return np.mean(losses), np.mean(accuracies)
...
def kfold_run(FLAGS, config_dict):
    ...
    acc, gyr, mag, targets = ReadData()._load_processed_data(
        config_dict['PREPROCESSED_ARR'])
    ...
    # results dataframe
    res_df = pd.DataFrame(
        np.empty((kf.n_splits, 4)),
        columns=['train_loss', 'train_acc', 'eval_loss', 'eval_acc'],
        index=[f'fold{fold+1}' for fold in range(kf.n_splits)])

    for fold, (train_idx, eval_idx) in enumerate(kf.split(targets)):
        print(f'\nfold: {fold+1}\n')

        train_loader = GaitDataLoader(acc[train_idx],
                                      gyr[train_idx],
                                      mag[train_idx],
                                      targets[train_idx],
        ...
        eval_loader = GaitDataLoader(acc[eval_idx],
                                     gyr[eval_idx],
                                     mag[eval_idx],
                                     targets[eval_idx],
        ...
        for epoch in range(FLAGS['EPOCHS']):
            # training
            gc.collect()
            train_loss, train_acc = train_func(model, train_loader, epoch,
                                               optimizer, loss_func)
            eval_loss, eval_acc = eval_func(model, eval_loader, loss_func)
