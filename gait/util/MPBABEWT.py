...
def ewt_meyer_scaling(w1, gamma, n):
    ...
    for i in range(n):
        if aw[i] <= mbn:
            ...
            yms[i] = np.cos(PI * ewt_beta(an * (aw[i] - mbn)) / 2)
...
def ewt_meyer_wavelet(wn, wm, gamma, n):
    ...
    for i in range(n):
        if (aw[i] >= pbn) and (aw[i] <= mbm):
            ...
            ymw[i] = np.cos(PI * ewt_beta(am * (aw[i] - mbm)) / 2)
        elif (aw[i] >= mbn) and (aw[i] <= pbn):
            ymw[i] = np.sin(PI * ewt_beta(an * (aw[i] - mbn)) / 2)
...
def ewt_meyer_filter_bank(boundaries, n):
    ...
    for i in range(len_boundaries - 1):
        r = (boundaries[i + 1] - boundaries[i]) / (boundaries[i + 1] +
                                                   boundaries[i])
        if r < gamma:
            gamma = r

    r = (PI - boundaries[len_boundaries - 1]) / (
        PI + boundaries[len_boundaries - 1])
    if r < gamma:
        gamma = r
    ...
    for i in range(len_boundaries - 1):
        mfb[:, i + 1] = ewt_meyer_wavelet(boundaries[i], boundaries[i + 1],
                                          gamma, n)

    mfb[:, len_boundaries] = ewt_meyer_wavelet(boundaries[len_boundaries - 1],
                                               PI, gamma, n)
...
def mode_eval_each_channel(zz, mfb):
    ...
    ewt = np.zeros((mfb.shape[1], mfb.shape[0]),
                   dtype='complex_')  # ewt.shape = 5*300
...
combined = np.sum(x, axis=1) / np.sqrt(3)

ff = np.fft.fft(combined)

frequencies = np.array([5, 10, 20, 30])

boundaries = np.array([(frequency * (2 * PI)) / fs
