...
log_level = config_dict['LOG_LEVELS'].get(
    os.environ.get('LOG_LEVEL')) if os.environ.get(
        'LOG_LEVEL') is not None else logging.WARNING

formatter = logging.Formatter(
    "[%(levelname)s] - %(pathname)s:%(lineno)d - %(asctime)s - %(name)s - : %(message)s"
)
